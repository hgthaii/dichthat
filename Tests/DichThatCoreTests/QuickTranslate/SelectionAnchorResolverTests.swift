...
@Test("Observed selection anchor takes precedence over later capture geometry")
func selectionAnchorResolution() {
    ...
    #expect(SelectionAnchorResolver.resolve(captured: bounds, observed: cursor) == cursor)
    #expect(SelectionAnchorResolver.resolve(captured: cursor, observed: nil) == cursor)
    #expect(SelectionAnchorResolver.resolve(captured: bounds, observed: nil) == bounds)
}
