...
func dictionaryRequestParameters() throws {
    ...
    #expect(values == ["t", "bd", "md", "rm", "ss", "ex"])
...
func enrichedParser() throws {
    let root: [Any] = [
        ...
        "en", NSNull(), NSNull(), NSNull(), 1, [], [], NSNull(), NSNull(),
        [["noun", [["setting", "background", "circumstances"]]]],
        [
    ...
    #expect(output.enrichment?.groups[0].synonyms == ["setting", "background", "circumstances"])
}

@Test("Enriched parser removes Google synonym metadata and internal IDs")
func enrichedParserFiltersSynonymMetadata() throws {
    let root: [Any] = [
        [["thuộc về thương mại", "commercial", NSNull(), NSNull(), 10]],
        [["adjective", ["thuộc về thương mại"], [], "commercial", 1]],
        "en", NSNull(), NSNull(), NSNull(), 1, [], [], NSNull(), NSNull(),
        [["adjective", [["business", "mercantile", "archaic", "m_en_gbus0203290.007"]]]],
    ]
    let data = try JSONSerialization.data(withJSONObject: root)
    let output = try #require(GoogleTranslationResponseParser.parseEnriched(
        data: data,
        expectedRoute: LanguageRoute(source: .english, target: .vietnamese, text: "commercial")
    ).success)
    #expect(output.enrichment?.groups[0].synonyms == ["business", "mercantile"])
}
