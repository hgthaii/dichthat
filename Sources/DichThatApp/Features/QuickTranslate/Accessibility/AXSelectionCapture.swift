...
final class AXSelectionCapture {
    ...
    func selectionBounds(frontmostPID: pid_t?) -> CaptureBounds? {
        let systemWide = AXUIElementCreateSystemWide()
        AXUIElementSetMessagingTimeout(
            systemWide,
            AppConfiguration.Accessibility.messagingTimeout
        ...
        if let focused = elementAttribute(kAXFocusedUIElementAttribute as CFString, on: systemWide),
           let bounds = directSelectionBounds(on: focused) {
            return bounds
        }
        ...
        roots.append(application)
        return descendantSelectionBounds(roots: roots)
    }

    private func directSelectionBounds(on element: AXUIElement) -> CaptureBounds? {
        AXUIElementSetMessagingTimeout(
            element,
            AppConfiguration.Accessibility.messagingTimeout
        )
        if let range = attribute(kAXSelectedTextRangeAttribute as CFString, on: element),
           let value = parameterizedAttribute(
               kAXBoundsForRangeParameterizedAttribute as CFString,
               parameter: range,
               on: element
           ), let rect = decodeRect(value), !rect.isEmpty {
            return captureBounds(rect)
        }
        if let markerRange = attribute(selectedMarkerRange, on: element),
           CFGetTypeID(markerRange) == AXTextMarkerRangeGetTypeID(),
           let value = parameterizedAttribute(
               boundsForMarkerRange,
    ...
    private func descendantSelectionBounds(roots: [AXUIElement]) -> CaptureBounds? {
        var budget = Budget()
        ...
        while index < queue.count {
            ...
            if budget.beginCall(), let bounds = directSelectionBounds(on: element) {
                return bounds
            }
            guard depth < AppConfiguration.Accessibility.maximumDepth,
                  budget.beginCall(),
                  let children = attribute(kAXChildrenAttribute as CFString, on: element) as? [AXUIElement]
            else { continue }
            let capacity = max(0, AppConfiguration.Accessibility.maximumNodes - queue.count)
            queue.append(contentsOf: children.prefix(capacity).map { ($0, depth + 1) })
        }
        return nil
    }

    private func captureBounds(_ rect: CGRect) -> CaptureBounds {
        CaptureBounds(
            x: rect.origin.x,
            y: rect.origin.y,
