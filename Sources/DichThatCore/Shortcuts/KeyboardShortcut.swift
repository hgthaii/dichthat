...
public struct KeyboardShortcut: Codable, Equatable, Sendable {
    ...
    public static func keyCode(forAlphanumeric character: String) -> UInt32? {
        let normalized = character.trimmingCharacters(in: .whitespacesAndNewlines).uppercased()
        guard normalized.count == 1,
              normalized.unicodeScalars.allSatisfy({
                  (65 ... 90).contains($0.value) || (48 ... 57).contains($0.value)
              })
        else { return nil }
        return keyDisplayNames.first(where: { $0.value == normalized })?.key
    }

    public var alphanumericKey: String? {
        guard let value = Self.keyDisplayNames[keyCode],
              Self.keyCode(forAlphanumeric: value) != nil
        else { return nil }
        return value
    }
