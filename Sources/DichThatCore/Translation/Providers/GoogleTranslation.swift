...
public enum GoogleTranslationRequestBuilder {
    ...
    public static func request(
        ...
        if mode == .dictionary {
            queryItems.append(contentsOf: ["bd", "md", "rm", "ss", "ex"].map {
...
public enum GoogleTranslationResponseParser {
    ...
    private static func parseOptionalEnrichment(
        ...
        if root.indices.contains(11), let synonymGroups = root[11] as? [Any] {
            for rawGroup in synonymGroups {
                guard let group = rawGroup as? [Any],
                      group.indices.contains(1),
                      let partOfSpeech = nonemptyString(group[0])
                else { continue }
                var excluded = Set([partOfSpeech.lowercased()])
                if let sourceText = expectedSourceText(from: segments) {
                    excluded.insert(sourceText.lowercased())
                }
                let synonyms = uniqueStrings(in: group[1]).filter {
                    !excluded.contains($0.lowercased()) && isPlausibleEnglishSynonym($0)
                }
                guard !synonyms.isEmpty else { continue }
                if let index = groups.firstIndex(where: {
                    $0.partOfSpeech.caseInsensitiveCompare(partOfSpeech) == .orderedSame
                }) {
                    groups[index] = groups[index].replacing(synonyms: synonyms)
                }
            }
        }
    ...
    private static func expectedSourceText(from segments: [Any]) -> String? {
        segments.compactMap { raw -> String? in
            guard let segment = raw as? [Any], segment.indices.contains(1) else { return nil }
            return nonemptyString(segment[1])
        }.joined().nilIfEmpty
    }

    private static func uniqueStrings(in value: Any) -> [String] {
        var result: [String] = []
        var seen: Set<String> = []
        func visit(_ value: Any, depth: Int) {
            ...
            if let string = nonemptyString(value) {
                ...
                if seen.insert(key).inserted { result.append(string) }
                return
            }
    ...
    private static func isPlausibleEnglishSynonym(_ value: String) -> Bool {
        let normalized = value.trimmingCharacters(in: .whitespacesAndNewlines).lowercased()
        let metadataLabels: Set<String> = [
            "archaic", "dated", "formal", "informal", "literary", "obsolete",
            "rare", "slang", "technical", "trademark",
        ]
        guard !metadataLabels.contains(normalized),
              !normalized.contains("_"),
              normalized.split(whereSeparator: \Character.isWhitespace).count <= 4
        else { return false }
        let allowed = CharacterSet.letters
            .union(.whitespaces)
            .union(CharacterSet(charactersIn: "'-"))
        return !normalized.isEmpty && normalized.unicodeScalars.allSatisfy(allowed.contains)
    }
}
